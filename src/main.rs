...
use alerting::{ResourceAlertRule, ContainerAlertRule, ResourceThreshold};
use notification::{NotificationSender, EmailNotifier, SlackNotifier};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let notifiers: Vec<Arc<dyn NotificationSender + Send + Sync>> = vec![
        Arc::new(email_notifier),
        Arc::new(slack_notifier),
    ];
